...
await replaceRequired("README.md", [
  [/\- Independent paid delivery: \[`PAID_DELIVERY_SUBMITTED_AWAITING_SETTLEMENT`\][^\n]*/, `- Independent paid delivery: [\`PAID_DELIVERY_SETTLED\`](./evidence/live/agent-studio-paid-delivery.json) — buyer \`0x2CB7…1Afa\`, Job \`1254\`, \`0.01 U\`, content-addressed \`PROTECTED\` result, and buyer approval after the canonical dispute window; [settlement transaction](https://testnet.bscscan.com/tx/${transactionHash})`],
  ...
  [/\| BNB Agent Studio Watchtower \| `PAID_DELIVERY_SUBMITTED_AWAITING_SETTLEMENT` \|[^\n]*/, "| BNB Agent Studio Watchtower | `PAID_DELIVERY_SETTLED` | runtime `running (ready)`, ERC-8004 ID `2447`; independent buyer funded Job `1254` with `0.01 U`, Agent submitted `PROTECTED`, and buyer approval completed after the canonical dispute window; B402 remains dormant |"]
]);

await replaceRequired("docs/ROADMAP.md", [
  ["- [ ] Approve Job `1254` after the canonical dispute window closes on September 19, 2026 at 20:07:31 Beijing time", "- [x] Approve Job `1254` after the canonical dispute window closes on September 19, 2026 at 20:07:31 Beijing time"]
]);

await replaceRequired("docs/JUDGE-GUIDE.md", [
  [/12\. In the paid-delivery artifact,[^\n]*/, `12. In the paid-delivery artifact, confirm the independent buyer \`0x2CB7…1Afa\`, Job \`1254\`, the content-addressed \`PROTECTED\` deliverable, and the buyer-approval transaction \`${transactionHash}\`; the job must read \`COMPLETED\`.`],
...
await replaceRequired("docs/VIDEO-SCRIPT.md", [
  [/\- Show the verified BSC Testnet managed-trial deployment,[^\n]*/, "- Show the verified BSC Testnet managed-trial deployment, Agent/Deployment IDs, ERC-8004 ID, public Agent Card, signed quote, and Job `1254` settlement receipt. Say “independent buyer funded, Agent delivered, buyer approved after the canonical dispute window”; do not claim B402 settlement."]
]);

await replaceRequired("site/index.html", [
  [/PAID JOB 1254 · (?:loading chain state|SUBMITTED · awaiting settlement) ↗/, "PAID JOB 1254 · COMPLETED · settlement verified ↗"],
  ...
  ["Final buyer settlement remains time-gated and is not claimed.", "The independent buyer settlement is backed by its own BSC Testnet receipt; B402 settlement is not claimed."]
]);
